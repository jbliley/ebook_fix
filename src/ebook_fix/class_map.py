...
def write_mapping_file(profiles: dict, path) -> None:
    ...
    This only produces the file for a person to review and edit --
    nothing reads it back yet, since there's no repair module to
    consume it. Once one exists, its input format is this file:
    old_name/role/new_name per [[class]] block.
    """
    ordered = sorted(profiles.values(), key=lambda p: -p.usage_count)
    ...
    lines = [
        ...
        "# Generated by `ebook-fixer map-css --write-mapping`.",
        "#",
        ...
        "#",
        "# NOTE: no repair module reads this file yet -- it's a review",
        "# step ahead of that being built.",
        "",
    ]
